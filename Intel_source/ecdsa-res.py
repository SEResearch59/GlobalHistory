...
if __name__ == '__main__':
    atks = os.listdir('.')
    ecdsa_dir = '/home/xiulau/Documents/myModules/ECDSA_RAND/' # check it before use
    keys = os.listdir('/home/xiulau/Documents/myModules/ECDSA_RAND/')
    atk_res = []
    key_res = []
    for str in atks:
        if str.find('rand_attack_ecdsa') != -1 and str.endswith('.txt'):
            atk_res.append(str)
    for str in keys:
        if str.find('rand_attack_ecdsa') != -1 and str.endswith('.txt'):
